...
- Subjects: component and component-instance commands/events under `*.component-service.*.*`, including compute results on `*.component-service.*.function_result.evt.component.compute_function.v1.{type}`
...
- `*.component-service.*.function_result.evt.component.compute_function.v1.{type}` — marks a data state as provided.
...
conn.publish('prod.component-service._.function_result.evt.component.compute_function.v1.data', JSON.stringify({
  data: { instanceId: 'inst-1', stateId: '<state-vertex-id>', name: 'input', type: 'data', result: { /* any */ } }
}));
```
